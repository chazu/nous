...
**5.5: Per-type operations** -- COMPLETE (2026-04-21)
Op categories added as Category units in `domains/math/op_categories.cue`: StrucOp, SetOp, BagOp, ListOp, OSetOp, MultEleStrucOp, OrdStrucOp, UnitOp. Each carries a `generalizations` chain mirroring EURISKO (ListOp ⊂ MultEleStrucOp, OrdStrucOp, StrucOp; OSetOp ⊂ OrdStrucOp ⊂ StrucOp; etc.). Existing concrete ops (SetUnion/Intersect/Difference, OSetUnion/Intersect/Insert/Delete/Equal, FirstEle/LastEle/AllButFirst/AllButLast/Proj1/Proj2) tagged with their appropriate per-type category in isA; `store.IsA` walks `isA` + `generalizations` so `SetUnion isA StrucOp` resolves. **Deferred tail landed 2026-04-21:** 14 concrete List/Bag op units added in `domains/math/list_bag_ops.cue` — List family (ListInsert/Delete/Delete1/Union/Intersect/Difference/Equal) and Bag family (BagInsert/Delete/Delete1/Union/Intersect/Difference/Equal). Backed by 9 new DSL builtins (`list-concat`, `list-equal?`, `list-remove-all`, `list-remove-one`, `list-intersect-ord`, `list-diff-ord`, `bag-equal?`, `bag-intersect`, `bag-diff`) in `internal/dsl/builtins.go`. Bag ops use multiset semantics (counts matter, order doesn't) via `map[string]int` count keyed by `v.AsString()`. Smoke coverage: `TestPhase55ListBagOpsComplete` in `internal/engine/engine_test.go`.
...
- **D' InvertOp** -- COMPLETE (structural parity, 2026-04-21). `invert-op` DSL builtin creates `Invert-<op>-<N>` with swapped domain↔range and NO defn (the inverted op is a structural discovery marker, not executable — real inverse search remains deferred per EURISKO's own low-worth treatment). Tagged `InvertedOp` category. New `H-Invert` heuristic (worth 100, matching EURISKO) fires on any Op with a defn and `invertRan` unset; creates the shell via `invert-op`; tagged `MetaOpHeuristic` so H-SemanticDup's gate auto-admits it. InvertOp + InvertedOp seed units added to `domains/math/operations.cue`. Behavior-level inverse algorithms deferred — meta-op discovery can reason about the structural shells without them.
