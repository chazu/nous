...
func TestSafePanelReportIsDeterministicAndCannotNameProtectedPanel(t *testing.T) {
	...
	if !first.MechanicallyValid || !first.DualExecutionEqual || !first.TranscriptHashesEqual || !first.Conservation || !first.OracleParity || !first.ProgramsExact || !first.ApplicationsExact || !first.ArtifactFrozen || !first.HeldoutSealed || len(first.Rows) != 9*len(empiricalPolicies) || first.Competence.Passed != true {
