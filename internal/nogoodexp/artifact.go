...
func FreezeArtifact(run TrainingRun) (FrozenArtifact, []byte, ArtifactAuthority, error) {
	...
	for _, evidenceName := range candidate.GetStrings("evidenceUnits") {
		...
		if evidence == nil || example == nil || bindingUnit == nil || barrier == nil || !barrier.GetBool("sealed") {
			return FrozenArtifact{}, nil, ArtifactAuthority{}, fmt.Errorf("incomplete training evidence %q", evidenceName)
		}
		record := FrozenEvidence{
			Example: example.Name, ProblemDigest: digestBytes([]byte(example.GetString("problem"))),
			...
			Matches: evidence.GetBool("matches"), AllConflict: evidence.GetBool("allConflict"),
			ExpectedKeys: slices.Clone(barrier.GetStrings("expectedKeys")), ActualKeys: slices.Clone(barrier.GetStrings("actualKeys")), Barrier: barrier.Name,
		}
		...
		artifact.Evidence = append(artifact.Evidence, record)
	}
	artifact.EvidenceBoundaryDigest = digestJSON(struct {
		Selection string           `json:"selection"`
		Barrier   string           `json:"barrier"`
		Evidence  []FrozenEvidence `json:"evidence"`
	}{artifact.Selection, artifact.SelectionBarrier, artifact.Evidence})
	artifact.CompletionDigest = digestJSON(artifact.Evidence)
	...
	for _, proofName := range u.GetStrings("promotionProofs") {
		...
		if proof == nil || promotionCase == nil {
...
func (artifact FrozenArtifact) Validate() error {
	...
	if len(artifact.Evidence) != 4 || len(artifact.PromotionProofs) != 24 || artifact.Digest != artifactDigest(artifact) {
	...
	for _, proof := range artifact.PromotionProofs {
		if proof.Proof == "" || proof.Case == "" || proofs[proof.Proof] || cases[proof.Case] || !proof.Conflict || !validDigest(proof.ProblemDigest) {
			return fmt.Errorf("invalid promotion proof set")
		}
		proofs[proof.Proof], cases[proof.Case] = true, true
	}
