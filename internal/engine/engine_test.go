...
// (which isA OrdStruc at instance level) and record an applic.
// Regression guard against breaking OrdStruc domain dispatch or losing
...
func TestFirstEleAppliedToOSetOfPrimesDesc(t *testing.T) {
	...
	if !found {
		t.Errorf("No FirstEle applic on OSetOfPrimesDesc; applics=%v", applics)
	}
}
